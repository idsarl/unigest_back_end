...
public class AuthController {
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequest loginRequest) {
        ...
        if (user.getRole() != Role.ADMIN
                && user.getRole() != Role.ENSEIGNANT
                && user.getRole() != Role.COMPTABLE) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(Map.of("message", "Accès non autorisé pour ce rôle"));
        }
        ...
        // Retourner l'ID de l'enseignant pour le frontend
        if (user.getRole() == Role.ENSEIGNANT) {
            authData.put("enseignantId", user.getId());
        }
